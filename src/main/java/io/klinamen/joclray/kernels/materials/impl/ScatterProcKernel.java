...
public class ScatterProcKernel extends AbstractScatterKernel {

    public ScatterProcKernel(cl_context context) {
        super(context);
    }

    public ScatterProcKernel(cl_context context, long seed0, long seed1) {
...
//        )
        ...
        ElementSet<SurfaceElement<? extends Surface>> surfaces = getParams().getScene().getSurfaces(x -> x.getMaterial() instanceof ProceduralWood);

        cl_mem kdBuf = track(OpenCLUtils.allocateReadOnlyMem(getContext(), surfaces.getFloatVec4sById(x -> ((ProceduralWood) x.getSurface().getMaterial()).getKd())));
