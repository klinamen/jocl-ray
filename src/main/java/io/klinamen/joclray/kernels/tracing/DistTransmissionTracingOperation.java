...
public class DistTransmissionTracingOperation extends AbstractOpenCLOperation implements OpenCLKernel<TransmissionTracingOperationParams> {
    private final cl_context context;
    private final IntersectionOperation intersection;
    private final BlinnPhongKernel shading;
    private final int maxGeneration;
    private final SplitRaysDistKernel splitRaysDist;
    
    private TransmissionTracingOperationParams params;
    
    public DistTransmissionTracingOperation(cl_context context, IntersectionOperation intersection, SplitRaysDistKernel splitRaysDist, BlinnPhongKernel shading, int maxGeneration) {
    ...
    protected void process(cl_command_queue queue, RaysGen source, IntersectionKernelBuffers intersectionKernelBuffers, TransmissionPropsBuffers transmissionPropsBuffers, BlinnPhongMaterialPropsBuffers materialPropsBuffers) {
        ...
        for (int i = 0; i < splitRaysDist.getSamples(); i++) {
            System.out.println(String.format("Reflection/Transmission sample %d/%d", i + 1, splitRaysDist.getSamples()));

            intersect(queue, source.getBuffer().getRaysBuffers(), intersectionKernelBuffers);
