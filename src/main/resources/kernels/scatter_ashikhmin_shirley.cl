...
__kernel void scatter_ashikhmin_shirley(
  ...
  float hit_nu = mat_nu[hit_id];
  float hit_nv = mat_nv[hit_id];
  ...
  float4 hit_throughput = rl_brdf_sample(z0, z1, n, u, v, -rd, &bd, hit_nu, hit_nv, hit_kd, hit_kr);
  ...
  if(ray == DEBUG_RAY){
    ...
    printf("scatter_ashikhmin_shirley: ray=%d, q_index=%d, q_size=%d, hm=%d, hit_kd=(%f, %f, %f), hit_emission=(%f, %f, %f), ro=(%f, %f, %f), rd=(%f, %f, %f), bo=(%f, %f, %f), bd=(%f, %f, %f), z0=%f, z1=%f, throughput=(%f, %f, %f), radiance=(%f, %f, %f)\n", 
